 ...
 * no tooltips, no library. Just "who got recognized, who had deductions,"
            ...
            <div key={row.name} className="flex h-full flex-1 flex-col items-center justify-end gap-1">
              <div className="flex h-full items-end gap-1">
                <div
                  ...
                  style={{ height: `${(row.positive / max) * 100}%` }}
                  title={`${row.name}: ${row.positive} positive`}
                />
                <div
                  className="w-4 rounded-t bg-negative"
                  style={{ height: `${(row.negative / max) * 100}%` }}
                  title={`${row.name}: ${row.negative} negative`}
                />
              </div>
